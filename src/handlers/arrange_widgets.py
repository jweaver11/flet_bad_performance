...
def arrange_widgets():
    ...
    # Use slice to avoid modifying list while iterating
        if hasattr(obj, 'pin_location') and obj.pin_location != "top":  # Check if pin_location does not match its actual location
            story.top_pin.tabs.remove(obj)  # Remove from top pin if not matching
    ...
    for obj in story.left_pin.tabs[:]:
        if hasattr(obj, 'pin_location') and obj.pin_location != "left":
            story.left_pin.tabs.remove(obj)
        ...
        if len(story.top_pin.tabs) > 0: 
            for obj in story.top_pin.tabs[:]:  
                if obj.visible == True:     # If there is at least one visible widget
                    ...
                    story.main_pin.tabs.append(obj)  # Add object to main pin
                    #story.main_pin.update()
                    story.top_pin.tabs.remove(obj)  # Remove it from top pin
                    #story.top_pin.update()
                    print("Stole from top pin")
                    break   # Exit our for loop
        # Check left pin
        elif len(story.left_pin.tabs) > 0:  # If top is empty, check left
            for obj in story.left_pin.tabs[:]:
                if obj.visible == True:
                   ...
                   #story.main_pin.update()
                    ...
                    break
        # Check right pin
        elif len(story.right_pin.tabs) > 0:  # If top and left are empty, check right
            for obj in story.right_pin.tabs[:]:
                if obj.visible == True:
                    obj.pin_location = "main"
                    story.main_pin.tabs.append(obj)
                    ...
                    break
        # Check bottom pin
        elif len(story.bottom_pin.tabs) > 0:  # If top, left, and right are empty, check bottom
            for obj in story.bottom_pin.tabs[:]:
                if obj.visible == True:
                    obj.pin_location = "main"
                    story.main_pin.tabs.append(obj)
