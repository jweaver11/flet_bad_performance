...
def arrange_widgets():
    ...
    def update_pin_location(obj):
        ...
        if pin_location == "top":
            app.active_story.top_pin.controls.append(obj)
        elif pin_location == "left":
            app.active_story.left_pin.controls.append(obj)
        elif pin_location == "main":
            app.active_story.main_pin.controls.append(obj)
        elif pin_location == "right":
            app.active_story.right_pin.controls.append(obj)
        elif pin_location == "bottom":
            app.active_story.bottom_pin.controls.append(obj)

    # Check our 5 pin locations, and if objects pin locations don't match their actual location, update/move them
    # Top
    for obj in app.active_story.top_pin.controls[:]:  # Use slice to avoid modifying list while iterating
        if hasattr(obj, 'data') and obj.data and obj.data['pin_location'] != "top":  # Check if pin_location does not match its actual location
            app.active_story.top_pin.controls.remove(obj)  # Remove from top pin if not matching
            update_pin_location(obj)    # Add it to the correct pin location
    # Left
    for obj in app.active_story.left_pin.controls[:]:
        if hasattr(obj, 'data') and obj.data and obj.data['pin_location'] != "left":
            app.active_story.left_pin.controls.remove(obj)
            update_pin_location(obj)
    # Main
    for obj in app.active_story.main_pin.controls[:]:
        if hasattr(obj, 'data') and obj.data and obj.data['pin_location'] != "main":
            app.active_story.main_pin.controls.remove(obj)
            update_pin_location(obj)
    # Right
    for obj in app.active_story.right_pin.controls[:]:
        if hasattr(obj, 'data') and obj.data and obj.data['pin_location'] != "right":
            app.active_story.right_pin.controls.remove(obj)
            update_pin_location(obj)
    # Bottom
    for obj in app.active_story.bottom_pin.controls[:]:
        if hasattr(obj, 'data') and obj.data and obj.data['pin_location'] != "bottom":
            app.active_story.bottom_pin.controls.remove(obj)
            update_pin_location(obj)
    ...
    def steal_from_other_pins():
        ...
        # Top
        if len(app.active_story.top_pin.controls) > 0:     # Pin has at least one widget
            for obj in app.active_story.top_pin.controls[:]:  
                if obj.visible == True and hasattr(obj, 'data') and obj.data:     # If at least one of the widgets is visible and has data
                    ...
                    app.active_story.top_pin.controls.remove(obj)  # Remove from top pin

                    # Exit our for loop, so it only does it to one widget, not all of them
                    break   
        # Left
        elif len(app.active_story.left_pin.controls) > 0: 
            for obj in app.active_story.left_pin.controls[:]:
                if obj.visible == True and hasattr(obj, 'data') and obj.data:
                    ...
                    break
        # Right
        elif len(app.active_story.right_pin.controls) > 0:  # If top and left are empty, check right
            for obj in app.active_story.right_pin.controls[:]:
                if obj.visible == True and hasattr(obj, 'data') and obj.data:
                    ...
                    break
        # Bottom
        elif len(app.active_story.bottom_pin.controls) > 0:  # If top, left, and right are empty, check bottom
            for obj in app.active_story.bottom_pin.controls[:]:
                if obj.visible == True and hasattr(obj, 'data') and obj.data:
                    obj.data['pin_location'] = "main"
                    update_pin_location(obj) 
    ...
    if len(app.active_story.main_pin.controls) == 0:   
    ...
    # If not empty, check if any of the objects are visible
        ...
        if all(obj.visible == False for obj in app.active_story.main_pin.controls[:]):
          steal_from_other_pins()
    ...
    def add_object_to_pin(obj):
        ...
        if hasattr(obj, 'data') and obj.data:  # If it has data
            if obj.data['pin_location'] == "top" and obj not in app.active_story.top_pin.controls:
                app.active_story.top_pin.controls.append(obj)
            elif obj.data['pin_location'] == "left" and obj not in app.active_story.left_pin.controls:
                app.active_story.left_pin.controls.append(obj)
            elif obj.data['pin_location'] == "main" and obj not in app.active_story.main_pin.controls:
                app.active_story.main_pin.controls.append(obj)  
            elif obj.data['pin_location'] == "right" and obj not in app.active_story.right_pin.controls:
                app.active_story.right_pin.controls.append(obj)
            elif obj.data['pin_location'] == "bottom" and obj not in app.active_story.bottom_pin.controls:
                app.active_story.bottom_pin.controls.append(obj)
