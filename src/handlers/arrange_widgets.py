from models.user import user

story = user.active_story  # Get our story object from the user

def arrange_widgets():
    print("arrange widgets called")

    # Append a reference obj to the pin location based on its pin location tag
    def update_pin_location(obj):
        pin_location = obj.pin_location
        if pin_location == "top":
            story.top_pin_obj.append(obj)
        elif pin_location == "left":
            story.left_pin_obj.append(obj)
        elif pin_location == "main":
            story.main_pin_obj.append(obj)
        elif pin_location == "right":
            story.right_pin_obj.append(obj)
        elif pin_location == "bottom":
            story.bottom_pin_obj.append(obj)

    # Check our five pin locations.
    # If any object in them has a pin location different from its actual location, move them
    for obj in story.top_pin_obj:
        if obj.pin_location != "top":
            story.top_pin_obj.remove(obj)  # Remove from top pin if not in top pin location
            update_pin_location(obj)    # Add it to the correct pin location
    for obj in story.left_pin_obj:
        if obj.pin_location != "left":
            story.left_pin_obj.remove(obj)
            update_pin_location(obj)
    for obj in story.main_pin_obj:
        if obj.pin_location != "main":
            story.main_pin_obj.remove(obj)
            update_pin_location(obj)
    for obj in story.right_pin_obj:
        if obj.pin_location != "right":
            story.right_pin_obj.remove(obj)
            update_pin_location(obj)
    for obj in story.bottom_pin_obj:
        if obj.pin_location != "bottom":
            story.bottom_pin_obj.remove(obj)
            update_pin_location(obj)


